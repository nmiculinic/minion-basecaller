...
if __name__ == "__main__":
    ...
    for k in [0.01, 0.1, 1, 1.0, 10.0, 100.0, 1000.0]:
        with tf.Graph().as_default():
            ...
            model.init_session()
            try:
                for i in range(model.restore(must_exist=False) + 1, 5001):
                    model.train_minibatch(log_every=10)
                    assert i == model.get_global_step()
                    if i % 100 == 0 or i in [1, 10, 20, 30, 40]:
