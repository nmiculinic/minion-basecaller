    ...
    for x in fnames:
        with gzip.open(x, "r") as f:
            ...
            for start in range(0, len(signal), cfg.seq_length):
                buff_idx = 0
                while basecall_idx < len(dp.basecalled) and dp.lower_bound[basecall_idx] < start + cfg.seq_length:
                    while basecall_squiggle_idx < len(dp.basecalled_squiggle) and dp.basecalled_squiggle[basecall_squiggle_idx] == dataset_pb2.BLANK: # Find first non-blank basecall_squiggle
                        basecall_squiggle_idx += 1

                    if basecall_squiggle_idx >= len(dp.basecalled_squiggle):
                        break
                    else:
                        buff[buff_idx] = dp.basecalled_squiggle[basecall_squiggle_idx]
                        buff_idx+=1
                        basecall_squiggle_idx +=1
                        basecall_idx += 1
                q.put([
