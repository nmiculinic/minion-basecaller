...
def process_mpileup(alignments_path, reference_path, mpileup_path, coverage_threshold, output_prefix):
    ...
    with open(mpileup_path, 'r') as fp:
        ...
        if output_prefix:
            ...
            variant_file = os.path.join(output_prefix, 'cov_%d.variant.csv' % coverage_threshold)
            fp_variant = open(variant_file, 'w')

            vcf_file = os.path.join(output_prefix, 'cov_%d.variant.vcf' % coverage_threshold)
        ...
        for line in tqdm(fp, total=n_lines):
            ...
            num_bases_to_skip, new_counts = process_mpileup_line(line, coverage_threshold, fp_variant, fp_vcf)
            counts += new_counts
...
def process_mpileup_line(line, coverage_threshold, fp_variant, fp_vcf):
    ...
    if coverage < coverage_threshold:
        ...
        variant_line = 'undercovered1\tpos = %s\tref = %s\tcoverage = %d\tbase_counts = %s\tinsertion_counts = %s\tdeletion_counts = %s' % (
            position, ref_name, int(coverage), str(sorted_base_counts), str(insertion_event_counts),
        ...
        if not is_good:
            if len(sorted_base_counts) > 0:
                ...
                position, ref_name, int(coverage), non_indel_coverage_current_base, most_common_base_count, ref_base,
                ('{}') if (len(sorted_base_counts) == 0) else (str(sorted_base_counts[-1][0])), str(sorted_base_counts),
    ...
    if (non_indel_coverage_next_base + deletion_count + insertion_count) > coverage_threshold:
        if len(insertion_event_counts.keys()) > 0:
            ...
            insertion_unique = True if (sum(
                [int(insertion_count[1] == most_common_insertion_count) for insertion_count in
                 sorted_insertion_counts]) == 1) else False
        else:
        ...
        if len(deletion_event_counts.keys()) > 0:
            ...
            deletion_unique = True if (sum([int(deletion_count[1] == most_common_deletion_count) for deletion_count in
                                            sorted_deletion_counts]) == 1) else False
        else:
        ...
        if most_common_insertion_count > most_common_deletion_count and most_common_insertion_count > non_indel_coverage_next_base:
            if insertion_unique:
                ...
                position, ref_name, non_indel_coverage_next_base, non_indel_coverage_current_base,
                most_common_insertion_count, ref_base, temp_sorted_bc, str(sorted_base_counts),
                str(insertion_event_counts), str(deletion_event_counts), line.strip())
                variant_write(variant_line)
    ...
    cnts = [snp_count, actual_insertion_count, actual_deletion_count,
