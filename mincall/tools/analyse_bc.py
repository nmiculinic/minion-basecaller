...
for name, cmd in basecallers.items():
    ...
    if os.path.isfile(sam_path):
        ...
        logging.info("Aligning %s to reference %s with graphmap", fasta_path, args.ref)
...
for col, ax_kde, ax_hist in zip(next(iter(dfs.values()))._get_numeric_data(), axes_kde.ravel(), axes_hist):
    for k in df.keys():
        sns.kdeplot(df[k][col], shade=True, label=k, alpha=0.5, ax=ax_kde)
        plt.hist(df[k][col], label=k, alpha=0.5, ax=ax_hist)
