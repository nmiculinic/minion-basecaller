...
As DNA strand pass through the nanopore, they are propelled by the current. However this current varies depending on specific DNA nucletide context within the nanopore, changing its resistance. We sample the curreny multiple times per second, 4000Hz in our dataset, and from this data we deduce passing DNA fragement. By design, nanopore is 6 nucleotides wide, and many models use this information internally, yet we opted our of it in hopes of simpler model, less feature engineering and better homopolymer detection.
...
\section{Basecalling}
The core of the decoding process is the basecalling step, that is translating the current sample to passed nucleotided sequence. Nowdays there's multiple basecalling options, what official and unoffical ones.
